    ...
    for window_size in window_sizes[windows_range]
        ...
        for selected_data in [k for k in keys(unique_cycles_count_in_windows[window_size])]
            ...
            for (img_index, img_name) = enumerate([k for k in keys(unique_cycles_count_in_windows[window_size][selected_data])])
                ...
                ax_q075 = CairoMakie.Axis(fgl[1, 1], ylabel="Q. 0.75", title=img_name)
                ax_q050 = CairoMakie.Axis(fgl[2, 1], ylabel="Q. 0.50",)
                ax_q025 = CairoMakie.Axis(fgl[3, 1], ylabel="Q. 0.25", xlabel="$(func)")
                ...
                for (k, data_vec, ax) = zip([0.75, 0.50, 0.25], [q075, q050, q025], ax_vec)
                    ...
                    ks_test_matrix = zeros(4, 4)
                    for row_index in 1:4
                        vec1 = data_vec[row_index].q_values
                        for col_index in 1:4
                            vec2 = data_vec[col_index].q_values
                            ks_test_matrix[row_index, col_index] = pvalue(ApproximateTwoSampleKSTest(vec1, vec2))
                        end
                    end
                    heatmap!(ax, centers_x, centers_y, ks_test_matrix, colorrange=(0, 1))
                    for i in 1:4, j in i:4
                        txtcolor = ks_test_matrix[i, j] < 0.5 ? :white : :black
                        value_str = Printf.@sprintf "%.3f" ks_test_matrix[j, i]
                        text!(ax, value_str, position=(i, j), color=txtcolor, align=(:center, :center))
                        if i != j
                            text!(ax, value_str, position=(j, i), color=txtcolor, align=(:center, :center))
                        end
                    end
                    f
            ...
            out_name = "$(scriptprefix)_p-values_at_quatiles_heatmap_$(data_stuff )"
            image_export_dir(args...) = plotsdir("section17", "$(scriptprefix)-pvalues_at_quartiles_heatmaps", "fixation_sequence=$(CONFIG.FIXATION_SEQUENCE)", args...)
