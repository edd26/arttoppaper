...
avg_land_dim0_group1, avg_land_dim0_group2, avg_land_dim1_group1, avg_land_dim1_group2 =
    map(x -> x.landscape |>
        ...
        [lanscapes_df_dim_0_group1, lanscapes_df_dim_0_group2, lanscapes_df_dim_1_group1, lanscapes_df_dim_1_group2
        ]
    )
...
ax_pland0_gr2 = CairoMakie.Axis(fgl2[1, 1]; title="Pseudo-art, dimension 0", pland_kwargs...)
...
ax_pland1_gr2 = CairoMakie.Axis(fgl4[1, 1]; title="Pseudo-art, dimension 1", pland_kwargs...)

fgl_vec = [fgl1, fgl2, fgl3, fgl4]
ax_vec = [ax_pland0_gr1, ax_pland0_gr2, ax_pland1_gr1, ax_pland1_gr2]
for ax in ax_vec
    CairoMakie.ylims!(ax, high=125)
end

colour_vec_dim0 = [:linear_kgy_5_95_c69_n256, :linear_kgy_5_95_c69_n256]
colour_vec_dim1 = [:sun, :sun]

land_vec = [avg_land_dim0_group1, avg_land_dim0_group2, avg_land_dim1_group1, avg_land_dim1_group2]
...
for (ax, fgl, pland, c) in zip(ax_vec, fgl_vec, land_vec, colour_vec)
    ...
    custom_colors = [RGBf(c) for c in cgrad(c, max(2, max_colour_range), categorical=true, rev=false,)]
...
map(x -> hidexdecorations!(x,
        ticks=false,
        grid=false,
    ),
    [ax_pland0_gr1, ax_pland0_gr2]
)
...
scriptprefix = "2gd2"
plot_2gd_dir(args...) = plotsdir("section2", scriptprefix * "-average-landscapes", "all_landscapes", args...)
# ===-===-===-
thr_folder = "threshold=$(PERSISTENCE_THRESHOLD)"

group_key = join(data_keys, "-")
joined_keys = lowercase(DATA_CONFIG) * "_" * group_key
out_name0 = plot_2gd_dir(thr_folder, joined_keys, "$(scriptprefix)_average_landscape_$(group_key).png")
safesave(out_name0, f1)

out_name1 = plot_2gd_dir(thr_folder, joined_keys, "pdf", "$(scriptprefix)_average_landscape_$(group_key).pdf")
