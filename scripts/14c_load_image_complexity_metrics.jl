...
data_sets = replace(data_names_vec, "JacksonPollock" => "Pollock")
...
if length(data_names_vec) < 6
    EdgeOrientation_df = vcat(map(
        x -> EdgeOrientation_folder(x, "export", "export-2023.07.28.csv") |> CSV.File |> DataFrame,
        data_names_vec
    )...)
else

    EdgeOrientation_df = vcat(map(
        x -> "EdgeOrientationEntropy_results_$(x).csv" |> EdgeOrientation_folder |> CSV.File |> DataFrame,
        data_names_vec
    )...)
end
...
EdgeOrientation_mod_df = hcat(
    DataFrame(
        image=[k |> split_and_replace for k in EdgeOrientation_df.image]),
...
land_areas_mod_df = hcat(
    DataFrame(
        image=[split(k, "_BW")[1] for k in filter(row -> row.dim == 0, land_areas_df)[:, :file]]
    ),
    DataFrame(
...
if !(land_areas_mod_df.image == FSlope_mod_df.image == PHOG_mod_df.image == EdgeOrientation_mod_df.image)
    ...
    all_data_df = hcat(
        ...
        [select(x, [k for k in names(x) if k != "set" && k != "image"]) for x in all_df_vec]...,
    )
end
