...
for (func_index, func) in parameters_vec[func_range] |> enumerate
    ...
    all_vals = vcat(func_related_df[:, selected_metric]..., func_related_df[:, selected_metric]...)
    ...
    ax_scatter1 = CairoMakie.Axis(fgl[1, 1], ylabel=scatter_label,
        xlabel=L"ME(ECDF_{looking}-ECDF_{not\;looking})",
    )
    ax_scatter2 = CairoMakie.Axis(fgl[1, 2], ylabel="",
        xlabel=L"ME(ECDF_{looking}-ECDF_{not\;looking})",
    )
    ...
    for (k, values_vec) in enumerate(measure_vals_both)
        ...
        if length(values_vec) > 0
            CairoMakie.violin!(ax_estimate, category_x_vals[not_nan_positions2], values_vec[not_nan_positions2]; color=(category_colour[k], violin_alpha), violin_kwargs...)
        end
    end
    #####################
    ...
    if pvalue(stat_test_result) < 0.05
        ...
        CairoMakie.bracket!(ax_boxplot, xtick_start, bracket_y, xtick_end, bracket_y, offset=0, text=p_marker, style=:square, fontsize=30, textoffset=-1)
    else
        p_marker = ""
    end # p-plot
...
image_export_dir(args...) = plotsdir("section17", "$(scriptprefix)-$(name_label)", "fixation_sequence=$(CONFIG.FIXATION_SEQUENCE)", args...)
