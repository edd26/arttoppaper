...
# ===-===-===-===-
df_info_storage(args...) = datadir("exp_pro", "section17", "$(scriptprefix)-ecdf_not_looking_BW_WB", "fixation_sequence=$(CONFIG.FIXATION_SEQUENCE)", args...)
...
img_name = [l for l in keys(unique_cycles_count_in_windows["BW"][window_size][selected_data])][5]
...
for func = parameters_vec[func_range]
    ...
    for window_size in window_sizes[windows_range]
        ...
        for selected_data in dataset_names[1:2]
            ...
            img_names = [l for l in keys(unique_cycles_count_in_windows["BW"][window_size][selected_data])]
            for img_name = img_names
                ...
                parameter_map = get_parameters_map_from_joined_filtration(parameter_map_BW, parameter_map_WB, func)
                parameters_values = get_parameters_values_from_joined_filtration(parameters_values_BW, parameters_values_WB, func)
                ...
                for session_index = 1:total_sessions, (view_index, view_indicator) = enumerate(view_range)
                    ...
                    ecdf_generation_config = @dict window_sizes unique_cycles_count_in_windows total_subjects window_size func selected_data et_df image_name session_index parameter
                    ...
                    ECDF_not_looking_data, p = produce_or_load(
                        ...
                        @unpack window_sizes, unique_cycles_count_in_windows, total_subjects, window_size, func, selected_data, et_df, image_name, session_index, parameter = ecdf_generation_config
                        ...
                        img_number = get_img_number(selected_data, image_name; data_config=CONFIG.DATA_CONFIG)
