...
for data_type in [:ECDF_mse, :ECDF_error, :KS_statistic]
    ...
    for func in parameters_vec[func_range]
        ...
        for selected_data in [k for k in keys(unique_cycles_count_in_windows[window_size])]
            ...
            all_vals = vcat(
                data_df[:, slection_looking_label]...,
                data_df[:, slection_notlooking_label]...,
            )
            ...
            for (img_index, img_name) in enumerate(img_list)
                ...
                img_parameters_df =
                    filter(row -> row.img_name == img_name, data_parameters_df)
                ...
                ax_scatter, ax_boxplot, ax_estimate =
                    set_up_ax_distro_plt(fgl1; scatter_label = scatter_label)
                do_distro_plot(
                    ax_scatter,
                    ax_boxplot,
                    ax_estimate,
                ...
                if selected_data == "art"
                    loaded_img =
                        preproc_img_dir("$(CONFIG.DATA_CONFIG)", "art", input_img_name) |>
                        load |>
                        channelview
                else
                    loaded_img =
                ...
                for subject in subjects
                    ...
                    windowed_heatmap_values =
                        [v for (k, v) in subject_ecdf_df.mean_heatmap_in_window[1]]
                    hist_weights =
                        windowed_heatmap_values[subject_ecdf_df.subject_looking[1]]

                    subject_looking_values = Float64[]
                    for (weight, value) in zip(
                        hist_weights,
                        img_parameters_df.parameters_values[1][subject_ecdf_df.subject_looking[1]],
                    )
                        for k = 1:ceil(weight)
                    ...
                    # ===-===-
                    empty_positions = [
                        p for (p, v) in subject_ecdf_df.mean_heatmap_in_window[1] if v == 0
                    ]
                    not_looked_values = filter(
                        x -> !isnan(x),
                        [
                            img_parameters_df.parameter_map[1][k[1], k[2]] for
                            k in empty_positions
                        ],
                    )
            ...
            image_export_dir(args...) = plotsdir(
                "section17",
                "$(scriptprefix)-$(main_name)",
                "$(CONFIG.DATA_CONFIG)_fixation_sequence=$(CONFIG.FIXATION_SEQUENCE)",
                args...,
            )
