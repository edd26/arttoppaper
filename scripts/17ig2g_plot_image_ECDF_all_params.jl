...
for func in parameters_vec[func_range]
    ...
    for (row, df1) in enumerate(eachrow(func_parameters_df))
        for (col, df2) in enumerate(eachrow(func_parameters_df))
            test_outcome, test_p_value = get_p_values(
                df1.parameters_values,
                df2.parameters_values;
                do_KW = do_KW,
...
for func in parameters_vec[func_range]
    ...
    ax_ecdf = CairoMakie.Axis(fgl[1, 1], xlabel = x_label, ylabel = y_label, title = "")
    ax_ecdf_art =
