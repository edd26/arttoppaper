...
for k in 1:total_metrics
    ...
    push!(axis_vec,
        CairoMakie.Axis(fgl[row, col]; args...)
    )
end
...
for (i, (ax, labels, data_arr, title)) in zip(
    [axis_fl, axis_selfsim, axis_complex, axis_pland0, axis_aniso, axis_avg_shannon_20_80, axis_edge_density, axis_pland1],
    [category_labels1, category_labels2, category_labels2, category_labels4, category_labels2, category_labels3, category_labels3, category_labels4],
    [data_array1, data_array2, data_array3, data_array7, data_array4, data_array5, data_array6, data_array8],
    ["Abs. Fourier slope", "Self-Similarity", "Complexity", "Persistence landscapes'\narea, dim 0", "Abs. Anisotropy", "Avg. Shannon20-80", "Edge density", "Persistence landscapes'\narea, dim 1"],
) |> enumerate
    ...
    if i == total_metrics
        ...
        x_label = ""
        hidexdecorations!(ax, label=true, ticklabels=false,
            ticks=false, grid=false, minorgrid=false, minorticks=false)
    end
    or = :vertical
...
script_prefix = "14ca"
plot_14c_dir(args...) = plotsdir("section14", script_prefix * "-metrics-comparison", args...)

savename_val = @savename useddata

if use_print_size
    out_name0 = plot_14c_dir("print", "$(script_prefix)_metrics_comparison_$(savename_val).png")
    safesave(out_name0, f)

    out_name1 = plot_14c_dir("print", "pdf", "$(script_prefix)_metrics_comparison_$(savename_val).pdf")
