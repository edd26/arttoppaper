...
# ===-===-===-===-
pvalues_info_storage(args...) = datadir("exp_pro", "section17", "$(scriptprefix)-rmse", "fixation_sequence=$(CONFIG.FIXATION_SEQUENCE)", args...)
        ...
        for selected_data in [k for k in keys(unique_cycles_count_in_windows[window_size])]
            ...
            for img_name = [k for k in keys(unique_cycles_count_in_windows[window_size][selected_data])]
                ...
                for session_index = 1:total_sessions, (view_index, view_indicator) = enumerate(view_range)
                    ...
                    pvalue_config = @dict window_sizes unique_cycles_count_in_windows total_subjects subjects_name window_size func selected_data et_df img_name session_index parameter view_indicator
                    if !isnan(default_map_value)
                        pvalue_config[:default_map_value] = default_map_value
                    end
                    ECDF_rmse_data, p = produce_or_load(
                        pvalues_info_storage("$(func)_sesion$(session_index)_view$(view_index)"), # path
                        ...
                        @unpack window_sizes, unique_cycles_count_in_windows, total_subjects, subjects_name, window_size, func, selected_data, et_df, img_name, session_index, parameter = pvalue_config
...
for func = parameters_vec[func_range]
    ...
    for d in ["art", "pseudoart"]
        ...
        session_index = 1
        view_index = 1
        for session_index = [1, 2], view_indicator in view_range
            ...
            for img_name in images
                ...
                for subject_df in eachrow(img_df)
                    ...
                    ecdf_mse = mse(ecdf_image(fixed_ecdf_range), ecdf_subject(fixed_ecdf_range))
                    ecdf_error = mean(ecdf_image(fixed_ecdf_range) .- ecdf_subject(fixed_ecdf_range))
                    ecdf_KS_manual = max(abs.(ecdf_image(fixed_ecdf_range) .- ecdf_subject(fixed_ecdf_range))...)

                    # push!(mse_img[d][session_index]["$(func)"][img_name], ecdf_mse)
                    # push!(mean_error_img[d][session_index]["$(func)"][img_name], ecdf_error)
                    total_items = length([v for (k, v) in subject_df.mean_heatmap_in_window])
                    total_looked_items = length([v for (k, v) in subject_df.mean_heatmap_in_window if v != 0])
                    total_not_looked_items = length([v for (k, v) in subject_df.mean_heatmap_in_window if v == 0])
