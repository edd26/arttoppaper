...
3b. Compile DIPHA with the following commands (DIPHA is a dependency included as a submodule):
   ...
   make
   ```
   Dipha is then used fror within Julia's script to obtain cubical complexes of images.
   
4. Python in version 3 with Numpy version 2.0 and above

Note: DrWatson package ensures that all the file management should work out of the box,
using repository structure for navigation. 

## Preprocessing

All images have to be converted into grayscale before being used in the repository.
...
create inversed, greyscale of the image, which is required for some of the scripts).
...
- fig 5. Produced with 17ga, 17id, 17ie2
